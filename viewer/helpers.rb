Viewer.helpers do
  ...
  def check_api_key
    ...
    api_key = @auth.username if @auth.provided?

    # ensure we have an API key provided
    halt 401, {:ok => false, :status => 401, :error => 'No API key provided'}.to_json unless api_key

    # check API key
    tenant = Tenant.with(:database => :ladder).find_by(:api_key => api_key) rescue nil
