class LadderHelper
  ...
  def self.dynamic_chunk(klass_or_collection)
    ...
    max_per_proc = (klass_or_collection.size.to_f / Parallel.processor_count.to_f)
    ...
    chunk_size = [([max_per_proc.to_f, max_per_free.to_f / Parallel.processor_count.to_f].min + 1).ceil, 10000].max
    num_chunks = (klass_or_collection.size.to_f / chunk_size.to_f).ceil
